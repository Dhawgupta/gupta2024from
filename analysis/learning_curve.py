...
def  plot(ax , data, label = None , color = None):
    ...
    mean = smoothen_runs(mean, factor = 0.99)
    stderr =  data['stderr'].reshape(-1)
    stderr = smoothen_runs(stderr)
    if color is not None:
        base, = ax.plot(list(range(len(mean)))[::500],mean[::500], label = label, linewidth = 3, color = color)
    else:
        base, = ax.plot(mean, label=label, linewidth=1)
    # base = ax.errorbar(list(range(len(mean))),mean, yerr=stderr, fmt='-', capsize=1, color='g', label='Data', alpha = 0.002)
    (low_ci, high_ci) = confidence_interval(mean, stderr)
    ax.fill_between(range(mean.shape[0])[::500], low_ci[::500], high_ci[::500], color = base.get_color(),  alpha = 0.2  )
...
for en, js in enumerate(json_handles):
    run, param , data = find_best(js, data = key_to_plot, metric = metric)
    print(param)
    agent = param['agent']    
