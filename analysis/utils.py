...
def load_different_runs_debug(json_handle):
    '''
    Format for json handle : Should not have any lists except for seed list to load different runs
    '''
    returns = []
    ...
    for i in iterable:
        ...
        try:
            ...
            arr = pkl_loader(filename)
            returns.append(arr['returns'])
            gammas.append(arr['gamma'])
            aux_returns.append(arr['aux_returns'])
            x1.append(arr['x1'])
            x3.append(arr['x3'])
            gradients.append(arr['gradients'])
            entropies.append(arr['entropies'])
    ...
    aux_returns = np.array(aux_returns)
    x1 = np.array(x1)
    x3 = np.array(x3)
    gradients = np.array(gradients)
    entropies = np.array(entropies)

    return returns, gammas, aux_returns, entropies, gradients


# FIXME change data = 'valid' to use the valie data for finding best
# def find_best(json_handle, data='mstde', key=None, metric='auc'):
#     '''
