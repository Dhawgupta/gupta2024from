...
class TDLambdaOnline:
    ...
    def update(self, x, a, xp, r, gamma, terminal):
        ...
        # convert history to a tensor
        x_ = torch.stack(self.history)
        v = self.model(x_)
        v_weight = torch.sum(v * (torch.tensor([ (gamma * self.lambda_) **(history_len - 1 - i) for i in range(history_len)]).reshape((history_len, 1))) )
        vgrad = torch.autograd.grad(v_weight, self.model.parameters(), create_graph=True)
