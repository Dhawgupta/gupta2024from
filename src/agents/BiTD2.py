...
class BiTD2:
    def __init__(self, params):
        ...
        # create a 1 layer neural network
        # self.model = nn.Sequential(nn.Linear(self.features, self.hidden_units), nn.ReLU(), nn.Linear(self.hidden_units, 2)).float()
        # self.model = nn.Sequential(nn.Linear(self.features, self.hidden_units), nn.ReLU(), nn.Linear(self.hidden_units, self.hidden_units), nn.ReLU(), nn.Linear(self.hidden_units, 2)).float()
    ...
    def getV_forward(self, state_features):
        return self.vf(state_features).detach().numpy()
    
    def getV_backward(self, state_features):
        return self.vr(state_features).detach().numpy()
    
    def getV_both(self, state_features):
        return self.vb(state_features).detach().numpy()
